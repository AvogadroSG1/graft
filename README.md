...
- **Paste-to-add** — `graft add [name]` ingests a full MCP JSON pasted on stdin (or `--file`), accepting both the wrapped `{"mcpServers":{…}}` form and a bare single-server object. It validates the JSON (rejecting unknown transport types, stdio servers without a command, and remote servers without a url), redacts literal secret-looking values into `${KEY}` env placeholders and reports the variable names, while keeping existing `${VAR}` references as-is. Pass `--force` to overwrite an existing definition.
